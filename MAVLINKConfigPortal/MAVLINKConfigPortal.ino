...
void setup()
{
  ...
  // Text box (String)
  WiFiManagerParameter custom_text_box("key_text", "Host IP Address", hostip, 50); // 50 == max length

  // Text box (Number)
  char convertedValue[6];
  sprintf(convertedValue, "%d", udpport); // Need to convert to string to display a default value.

  WiFiManagerParameter custom_text_box_num("key_num", "UDP Port", convertedValue, 7); // 7 == max length
 ...
 // wm.addParameter(&custom_checkbox);
  ...
  if (forceConfig)
  {
    if (!wm.startConfigPortal("Mini Crossbow Link", "password"))
    {
  ...
  {
    if (!wm.autoConnect("Mini Crossbow Link", "password"))
    {
      Serial.println("failed to connect and hit timeout");
      delay(3000);
  ...
  strncpy(hostip, custom_text_box.getValue(), sizeof(hostip));
  Serial.print("hostip: ");
  Serial.println(hostip);

  //Convert the number value
  udpport = atoi(custom_text_box_num.getValue());
